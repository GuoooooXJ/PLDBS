...
% STN beta: 0 180
%arraryBeta = [0	180	180	180	0	0	0	180]; % Random phase array
%arraryBeta = [90	-90	-90	-90	90	90	90	-90]; % Random phase array
arraryBeta = [20	-160	-160	-160	20	20	20	-160]; % Random phase array
...
%while isample < tDur*SR
while iStmPha<PulseNumber+1
...
%            end
        ...
        elseif isample == p+1
            ...
            if isample>windowSize*1/10*n  %% Find Target Phase
                   %if (abs(degree_-xTargetPhase)<(360*Frequency/SR/2)  && amplitude_>THRS ) && (cfg.Remove < 1) && (stCheck < 1 ) && (PreviousStimu < 1) && ((isample-LastStimuTime)>SR)
                   if (abs(degree_-xTargetPhase)<(360*Frequency/SR/2)) && (cfg.Remove < 1) && (stCheck < 1 ) && (PreviousStimu < 1) && ((isample-LastStimuTime)>SR)
