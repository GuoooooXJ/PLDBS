...
% Target Phase sequence (must be -180 to 180) / degree
% STN beta: 0 180
arraryBeta = [0	0	180	0	180	0	180	180]; % Random phase array
%arraryBeta = [-90	90	90	-90	-90	90	-90	90]; % Random phase array
%arraryBeta = [-160	20	20	-160	-160	20	-160 20]; % Random phase array
%arraryBeta = [0 90	180	-90];

%arraryBeta = arraryBeta(randperm(length(arraryBeta))); % Shuffle the sequence

TargetPhaseSeq = arraryBeta;

% Target Phase / degree
TargetPhase = TargetPhaseSeq(No);

%% Parameter setting (After run Beta_SetUp.m)

%Read parameters from config
%para = load("Beta_config.txt");
...
% in case delay is too long as more than one cycle
if xTargetPhase > 180
...
%            end
        ...
        elseif isample == p+1
            ...
            if raw ~= Prediction(end-1) && (isnan(raw) == 0)
...
%% Save data
filename = sprintf('Realtime_recording_Beta_180_0_CyleByCycle_%d.mat', No);
save(filename,"KlaDta","FltDta","RowDta","TtlDta","StmDta","PhaDta");
...
ylim([-ylm ylm]);
legend('KlaDta', 'FltDta'); legend('boxoff');
xlim([tmx(1) tmx(end)]);
...
plot(tmx,InsAmp,'-k'); grid on;
legend('Inst Amplitude'); legend('boxoff');
